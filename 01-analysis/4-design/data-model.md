...
> отдаёт её **постранично** (пагинация, `limit`/`offset`). При **удалении аккаунта**
> (`deleteAccount`) персональные данные клиента (`name`, `phone`) **анонимизируются**, а не
> удаляются жёстко: связанные брони сохраняются обезличенными (для учёта/статистики
> существующей инфраструктуры), `Client.phone` освобождается для повторной регистрации.
...
| start_at | datetime (UTC) | Дата и время старта в UTC; **источник истины — сервер**. Клиент отображает в локальном времени, но право/тип отмены (правило 2 часов) вычисляет сервер (R-021) |
