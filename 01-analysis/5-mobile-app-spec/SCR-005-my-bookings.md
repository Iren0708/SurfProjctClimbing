...
|----------|-----|----------------|----------|----------|
| `status` | string (`active`/`cancelled`/`late_cancel`) | Нет | — | Опциональный фильтр по статусу. На SCR-005 **не передаётся**: грузим **всю историю** (включая отменённые/late_cancel), разделение делаем на клиенте. |
...
**Ответ (200):** `BookingListResponse` = `items: BookingSummary[]` + `meta` (пагинация). Поля `BookingSummary`, используемые на экране: `id`, `seats_count`, `rental_count`, **`price_total`** (серверный итог, RUB), `status`, `cancelled_at`, `slot` (`SlotSummary` → `start_at`, `route.name`, `route.type`, `instructor.name`, тарифы `price`/`rental_price` для разбивки).
...
| AC-009 | **Дано** в истории есть отменённые/поздно отменённые брони (`cancelled`/`late_cancel`), **Когда** клиент открывает экран без фильтра по статусу, **Тогда** они присутствуют в истории («Прошедшие») с соответствующим бейджем — из выдачи не исключаются (R-025) | P1 |
